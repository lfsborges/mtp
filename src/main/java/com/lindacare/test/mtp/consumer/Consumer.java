package com.lindacare.test.mtp.consumer;

import com.google.common.collect.Lists;
import com.lindacare.test.mtp.configurations.KafkaConfig;
import com.lindacare.test.mtp.model.MessageDeserializer;
...
public class Consumer implements Managed {
  ...
  private final Queue<Message> queue = new ArrayBlockingQueue<>(CAPACITY);
    ...
    do {
      ...
      for (ConsumerRecord<String, Message> record : records) {
        ...
        if (queue.offer(record.value())) {
          ...
          LOG.error("Failed to track message: {}", record.value());
        }
      }
    } while (running.get());
